...
class ObjectRemove():
    ...
    def __init__(self, segmentModel=None, rcnn_transforms=None, inpaintModel=None, image_path='', detections=None):
    ...
    def run(self):
        ...
        print('user click')
        #user click
        print('Using detections for object removal')
        for detection in self.detections:
            # Convert detection to the format expected by find_mask if necessary
            # This example assumes detection is already in the correct format
            self.box = detection
            self.highest_prob_mask = self.find_mask(out, detection)
    ...
    # def smooth_mask1(self, mask, kernel_size=5): #morphological
    #     """
    #     Applies morphological operations to smooth the mask.
        
    #     Parameters:
    #     - mask: The binary mask to be smoothed, as a numpy array or a torch tensor.
    #     - kernel_size: Size of the morphological kernel.
        
    #     Returns:
    #     - smoothed_mask: The smoothed mask as a numpy array.
    #     """
    ...
    #     # Ensure mask is a single channel 2D array
    ...
    #     smoothed_mask = cv2.dilate(mask_eroded, kernel, iterations=1)

    #     return smoothed_mask

    # def smooth_mask(self, mask, kernel_size=7):  #gaussianblur
    ...
    #     smoothed_mask = cv2.GaussianBlur(mask_np, (kernel_size, kernel_size), sigmaX=3)
    #     # Threshold the mask again to ensure it is binary
    #     _, smoothed_mask = cv2.threshold(smoothed_mask, 0.5, 1, cv2.THRESH_BINARY)
    #     return smoothed_mask
